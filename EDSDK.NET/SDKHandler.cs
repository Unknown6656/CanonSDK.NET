...
#error todo fix this shite
        ...
        public List<int> GetSettingsList(PropID property)
        {
            if (MainCamera.Handle is 0)
                throw new InvalidOperationException("Camera or camera reference is null/zero");

            if (property is PropID.AEModeSelect or PropID.ISOSpeed or PropID.Av or PropID.Tv or PropID.MeteringMode or PropID.ExposureCompensation)
            {
                Error = EDSDK_API.EdsGetPropertyDesc(MainCamera.Handle, property, out EdsPropertyDesc des);

                return des.PropDesc.Take(des.NumElements).ToList();
            }
            else
                throw new ArgumentException("Method cannot be used with this Property ID", nameof(property));
        }
        ...
        public unsafe T GetSettingU32<T>(PropID property)
            where T : unmanaged
        {
            if (sizeof(T) != sizeof(uint))
                throw new ArgumentException($"The type {typeof(T)} must have a size of {sizeof(uint)} bytes - not {sizeof(T)} bytes.", nameof(T));
            else if (MainCamera.Handle is 0)
                throw new InvalidOperationException("Camera or camera reference is null/zero");

            Error = EDSDK_API.EdsGetPropertyData(MainCamera.Handle, property, 0, out uint value);
